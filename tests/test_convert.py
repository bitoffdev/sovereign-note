...
def cmpfiles(ax, bx):
    if ax[-5:] == ".cson":
        ...
        return FileComparison.EQUAL if recdiff(acontents, bcontents) else FileComparison.DIFFERENT
    elif ax[-5:] == ".json":
        with open(ax) as fh:
...
def assert_equal_boostnote(a, b):
    ...
    assert all(x in [FileComparison.EQUAL, FileComparison.IGNORE] for x in result.values())
